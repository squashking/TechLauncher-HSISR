...
def main():
    ...
    result_parser = subparsers.add_parser("result", help="parser for testing arguments")
    result_parser.add_argument("--cuda", type=int, required=False,default=1,
                             help="set it to 1 for running on GPU, 0 for CPU")
    result_parser.add_argument("--gpus", type=str, default="0,1", help="gpu ids (default: 7)")
    result_parser.add_argument("--dataset_name", type=str, default="Chikusei",help="dataset_name, default set to dataset_name")
    result_parser.add_argument("--model_title", type=str, default="MSDformer",help="model_title, default set to model_title")
    result_parser.add_argument("--n_feats", type=int, default=240, help="n_feats, default set to 240")
    result_parser.add_argument("--n_blocks", type=int, default=4, help="n_blocks, default set to 4")
    result_parser.add_argument("--n_subs", type=int, default=8, help="n_subs, default set to 8")
    result_parser.add_argument("--n_ovls", type=int, default=0, help="n_ovls, default set to 1")
    result_parser.add_argument("--n_scale", type=int, default=4, help="n_scale, default set to 2")
...
def train(args):
    ...
    train_path = './datasets32/'+args.dataset_name+'_x'+str(args.n_scale)+'/train/'
    eval_path = './datasets32/' + args.dataset_name + '_x' + str(args.n_scale) + '/val/'
    ...
    train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers=52, shuffle=True)
    eval_loader = DataLoader(eval_set, batch_size=args.batch_size, num_workers=52, shuffle=False)
    ...
    model_name = './checkpoints/' + "Sat Apr 27 09:28:40 2024/" + args.dataset_name + "_" + model_title + "_ckpt_epoch_" + str(110) + ".pth"
...
def get_fin_result(args):
    colors = 480
    fin_data_dir = './datasets32/' + args.dataset_name + '_x' + str(args.n_scale) + '/'
    result_path = './results/' + args.dataset_name + '_x' + str(args.n_scale) + '/'
    model_title = args.model_title +'_Blocks=' + str(args.n_blocks) + '_Subs' + str(args.n_subs) + '_Ovls' + str(args.n_ovls) +'_Feats=' + str(args.n_feats)
    #model_name = './checkpoints/' + 'Tue Apr  9 11:21:25 2024/'+ args.dataset_name +'_'+ model_title + "_ckpt_epoch_" + str(95) + ".pth"
    model_name = './checkpoints/Sun Apr 28 01:40:36 2024/appf_MSDformer_Blocks=4_Subs8_Ovls2_Feats=240_ckpt_epoch_115.pth'
    device = torch.device("cuda" if args.cuda else "cpu")
    #print('===> Loading testset')
    print(fin_data_dir)
