...
class UnsupervisedClassificationWorker(QThread):
    ...
    classification_finished = pyqtSignal(QPixmap, np.ndarray, np.ndarray)
    error_occurred = pyqtSignal(str)
    ...
    def run(self):
        try:
            ...
            # Emit the pixmap, cluster_map, and ndvi
            self.classification_finished.emit(pixmap, cluster_map, ndvi)
